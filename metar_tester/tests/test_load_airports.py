...
from metar_tester.load_airports import LoadAirports
...
class TestLoadAirports(TestCase):
    ...
    @mock.patch('metar_tester.load_airports.LoadAirports.is_valid', return_value=True)
    @mock.patch('metar_tester.load_airports.os')
    def test_load_airports(self, mock_os, mock_load_airports_is_valid):
        mock_os.path.join.return_value = os.path.join(os.getcwd(), 'metar_tester', 'tests', 'static', 'load_airports', 'sample_airports.csv')
    ...
    def test_load_airports_duplicates(self, mock_os, mock_load_airports_is_valid):
        mock_os.path.join.return_value = os.path.join(os.getcwd(), 'metar_tester', 'tests', 'static', 'load_airports', 'sample_airports_duplicates.csv')
        mock_load_airports_is_valid.return_value = True
        self.load_airports.main()
        self.assertEquals(len(Airport.objects.all()), len(self.sample_airport_icaos))
        self.assertContainsAirports()
    ...
    def test_load_airports_empty(self, mock_os, mock_load_airports_is_valid):
        mock_os.path.join.return_value = os.path.join(os.getcwd(), 'metar_tester', 'tests', 'static', 'load_airports', 'sample_airports_empty.csv')
        mock_load_airports_is_valid.return_value = True
        self.load_airports.main()
